...
class Deep(nn.Module):
    ...
    def __init__(
        ...
        self.SpatialGrid_size = (0.1, 0.1, 0.1)
        self.num_grids = 0 #图像网格数
        self.rays = [] #射线list
        self.num_SpatialGrids = (50, 50, 50)  # 空间网格数
    ...
    def compute_ray(self, i, poses, K):
        ...
        for grid in grids:
            ...
            camera_uv = np.array([(u - K[0, 2]) / K[0, 0], (v - K[1, 2]) / K[0, 0], 1, 0])
            # print("camera_uv", camera_uv)
            # 相机坐标系转换到世界坐标系
            transformed_camera_uv = np.dot(T[:3, :], camera_uv)
            ...
            direction = transformed_camera_uv[:3] - np.squeeze(transformed_camera_o[:3])
            origin = np.squeeze(transformed_camera_o[:3])
    ...
    def deep(self):
        ...
        for k in range(num_SpatialGrids):
            ...
            # if self.SpatialGrids[k].features.shape[0] > 0:
            if self.SpatialGrids[k].features is not None:
                print(k, "Sgrid", self.SpatialGrids[k].features)
                # if self.SVD(k, 0.5):
                    # print("loc", self.SpatialGrids[k].loc)
                deep.append(self.SpatialGrids[k].loc)
        print("deep", deep)
        self.visualization(deep, False, False)
        return deep
    ...
    def write(self, pcd):
        # 写入文件
        open3d.io.write_point_cloud(self.data_folder + "/point_cloud4.ply", pcd)
        with open(self.data_folder + "/svd4.txt", "w") as file:
    ...
    def read(self):
        # 从PLY文件加载点云数据
        ply = open3d.io.read_point_cloud(self.data_folder + "/point_cloud8_5.ply")
        # 将点云数据转换为 NumPy 数组
        points = np.asarray(ply.points)
        i = 0
        rpoint = []
        with open(self.data_folder + "/svd8_5.txt", "r") as file:
            # 循环读取文件中的每一行
            for line in file:
                # 处理每一行的数据
                data = line.split()
...
if __name__ == "__main__":
    a = Deep()
    feature = a.image_features('/home/liu/ForDeep/ForDeep/data/duck/images/duck_1.jpg')
    # print(feature[0, :, 20, 22])
    feature1 = a.image_features('/home/liu/ForDeep/ForDeep/data/duck/images/duck_16.jpg')
    feature2 = a.image_features('/home/liu/ForDeep/ForDeep/data/duck/images/duck_16.jpg')
    # print(feature[0, :, 22, 21])
    stacked_v = np.vstack((np.reshape(feature[0, :, 24, 24].detach().numpy(), (1, 256)), np.reshape(feature1[0, :, 24, 25].detach().numpy(), (1, 256)), np.reshape(feature2[0, :, 27, 25].detach().numpy(), (1, 256)), np.reshape(feature[0, :, 42, 28].detach().numpy(), (1, 256))))
    U, S, VT = np.linalg.svd(stacked_v)
    print("S", S)
    print(S.shape)
    print("f1", S[1] / S[0])
    print("f2", S[2] / S[1])
