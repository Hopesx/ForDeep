...
class Deep(nn.Module):
    ...
    def visualization(self, points, colors, iswrite, isread):
        ...
        # 创建坐标系
        coord_frame = open3d.geometry.TriangleMesh.create_coordinate_frame(size=20, origin=[0, 0, 0])
    ...
    def write(self, pcd):
        # 写入文件
        open3d.io.write_point_cloud(self.data_folder + "/result" + "/point_get.ply", pcd)
        with open(self.data_folder + "/result" + "/svd.txt", "w") as file:
    ...
    def read(self):
        # 从PLY文件加载点云数据
        ply = open3d.io.read_point_cloud(self.data_folder + "/result" + "/point_new1.ply")
        # 将点云数据转换为 NumPy 数组
        ...
        rpoint = []
        rcolor = []
        with open(self.data_folder + "/result" + "/svd_new1.txt", "r") as file:
            # 循环读取文件中的每一行
            for line in file:
                # 处理每一行的数据
                data = line.split()
