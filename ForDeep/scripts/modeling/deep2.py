...
class Deep(nn.Module):
    ...
    def visualization(self, point, iswrite, isread):
        ...
        # 创建坐标系
        coord_frame = open3d.geometry.TriangleMesh.create_coordinate_frame(size=20, origin=[5, 10, 15])
    ...
    def write(self, pcd):
        # 写入文件
        open3d.io.write_point_cloud(self.data_folder + "/point_cloud9_5.ply", pcd)
        with open(self.data_folder + "/svd9_5.txt", "w") as file:
    ...
    def read(self):
        ...
        i = 0
        rpoint = []
        with open(self.data_folder + "/svd9_5.txt", "r") as file:
            # 循环读取文件中的每一行
            for line in file:
                # 处理每一行的数据
                data = line.split()
                ...
                f2 = float(data[1])
                f3 = float(data[2])
                if f1 < 0.3 and f2 < 0.7 and f3 < 0.8:
                    print("f1", f1)
                    print("f2", f2)
                    print("f3", f3)
                    print(points[i])
