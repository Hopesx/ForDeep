...
class Deep(nn.Module):
    ...
    def __init__(
            self,
            image_encoder: ImageEncoderViT = ImageEncoderViT(576, 720),
        ...
        self.image_num = 27
        self.image_size = (576, 720)
        self.grid_size = (16, 16)
        self.SpatialGrid_size = (0.05, 0.05, 0.05)
        ...
        # self.overlaps = []
        self.center = (-0.1, 0, 0.6)
        self.data_folder = "/home/liu/ForDeep/ForDeep/data/Cerealboxes2"  # 数据文件夹路径
        self.feature_model = "/home/liu/ForDeep/ForDeep/LinearCorrNet/vae_model3/vae_model3.pth" # 特征处理模型路径
        ...
        if start is None:
            ...
            for i in range(3):
                ...
                for local[(i + 1) % 3], local[(i + 2) % 3] in itertools.product(range(min[(i + 1) % 3], max[(i + 1) % 3]), range(min[(i + 2) % 3], max[(i + 2) % 3])):
                    ...
                    if self.is_ray_intersect_grid(ray, local) and not (all(component <= 60 for component in color)):
                        # print("loc", local)
                        # 射线经过空间网格
                        find = True
    ...
    def write(self, pcd):
        # 写入文件
        open3d.io.write_point_cloud(self.data_folder + "/result" + "/point_new2.ply", pcd)
        with open(self.data_folder + "/result" + "/svd_new2.txt", "w") as file:
    ...
    def read(self):
        # 从PLY文件加载点云数据
        ply = open3d.io.read_point_cloud(self.data_folder + "/result" + "/point_new2.ply")
        # 将点云数据转换为 NumPy 数组
        ...
        rpoint = []
        rcolor = []
        with open(self.data_folder + "/result" + "/svd_new2.txt", "r") as file:
            # 循环读取文件中的每一行
            for line in file:
                # 处理每一行的数据
                data = line.split()
